...
# Check if the output contains any issues (exclude our own notifications)
if echo "$LOGWATCH_OUTPUT" | grep -qiE 'warning|error|critical|fail'; then
    # If issues found, send a notification
    sudo -u tassilo DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$(id -u tassilo)/bus notify-send "Logwatch Alert" "Potential issues detected in system logs. Check full report under $LATEST_LINK for details."
fi
