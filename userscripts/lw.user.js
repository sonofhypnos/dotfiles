...
// @downloadURL   https://raw.githubusercontent.com/sonofhypnos/dotfiles/main/userscripts/lw.user.js
// ==/UserScript==
...
(function () {
  ...
  async function replaceBookmarksSection() {
    ...
    const emptyMessage = bookmarksContainer.querySelector(
      'div[class*="empty"]',
    );
  ...
  async function enhanceLoadMoreButton() {
    ...
    loadMoreButton.parentNode.insertBefore(
      newButton,
      loadMoreButton.nextSibling,
    );
    ...
    newButton.addEventListener("click", async () => {
      ...
      const totalBookmarks =
        document
          .querySelector(".BookmarksList-root")
          ?.closest("[data-reactroot]")?.__reactInternalInstance$
          ?._currentElement?._owner?._instance?.props?.currentUser
          ?.bookmarkedPostsMetadata?.length || 0;

      // Keep clicking load more until all are loaded
      const loadMore = async () => {
        const visiblePosts =
          document.querySelectorAll(".PostsItem-root").length;
        if (
          loadMoreButton &&
          !loadMoreButton.disabled &&
          visiblePosts < totalBookmarks
        ) {
